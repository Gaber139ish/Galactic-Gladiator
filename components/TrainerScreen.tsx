...
const TrainerScreen: React.FC<TrainerScreenProps> = ({ player, trainerSkills, onLearnSkill, onBack }) => {
    ...
    const SkillCard = ({ skill }: { skill: Skill; }) => {
        ...
        return (
            <div className={`hud-card p-4 rounded-lg flex flex-col justify-between ${!canLearn && !isLearned ? 'opacity-60' : ''} ${isLearned ? 'border-green-500/50' : ''}`}>
                <div>
                ...
                <div className="mt-4">
                    {isLearned ? (
                        ...
                        <button 
                            ...
                            disabled={!canLearn}
                            className="w-full btn btn-success !py-2 text-sm"
                        >
                            Learn ({skill.cost} G)
                        </button>
                    )}
